...
class DefaultThemePublisher(Publisher):
    def _generate_codeblock_content(self, html_content: str, line: str, element: MarkdownElement) -> str:
        html_content = self._push_to_html_content(html_content, '<pre style="overflow: scroll;">')
        html_content = self._push_to_html_content(html_content, f'<code class="language-{element.extra["language"]}">', 1)
